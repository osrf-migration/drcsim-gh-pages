...
class Signer(QtCore.QThread):
  ...
  def __init__(self, text):
    ...
    self.text = text
  def run(self):
    cmd = "cd " + os.path.dirname(os.path.realpath(__file__)) + " && bash -c \". " + os.path.dirname(os.path.realpath(__file__)) + "/../../../../../devel/setup.bash && ./simple_grasp_asl.py only \\\"%s\\\" 4 1\" " % self.text
    print cmd
    os.system(cmd)
